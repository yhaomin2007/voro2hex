	...
	# only do this if not ghost face and non-collapsed face
		...
		if (not ifghost) and (not ifcollapse) and (ifchamfer): # special process to deal with polygon with chamfers
			...
			for iedge in range(0,nedges):
				...
				vc1xyz,moved = move_away_from_pebbles(vc1xyz,pxyz,pebble_diameter,1.01,ptag)
				vc2xyz,moved = move_away_from_pebbles(vc2xyz,pxyz,pebble_diameter,1.01,ptag)
				...
				vcmxyz =  line_split(pmiddle,m12,r_c/d_m12)
				vcmxyz,moved = move_away_from_pebbles(vcmxyz,pxyz,pebble_diameter,1.01,ptag)
